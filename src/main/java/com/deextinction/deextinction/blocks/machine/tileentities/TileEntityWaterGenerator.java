...
import com.deextinction.deextinction.blocks.machine.recipes.WaterGeneratorRecipes;
...
public class TileEntityWaterGenerator extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.watergenerator");
	}
	...
	public static boolean isBurning(TileEntityWaterGenerator te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			WaterGeneratorBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = WaterGeneratorRecipes.getInstance().getWaterGeneratorResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = WaterGeneratorRecipes.getInstance().getWaterGeneratorResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
