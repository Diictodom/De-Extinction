...
import com.deextinction.deextinction.blocks.machine.recipes.GrinderRecipes;
...
public class TileEntityGrinder extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.grinder");
	}
	...
	public static boolean isBurning(TileEntityGrinder te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			GrinderBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = GrinderRecipes.getInstance().getGrinderResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = GrinderRecipes.getInstance().getGrinderResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
