...
import com.deextinction.deextinction.blocks.machine.recipes.SolarRecipes;
...
public class TileEntitySolar extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.solar");
	}
	...
	public static boolean isBurning(TileEntitySolar te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			SolarBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = SolarRecipes.getInstance().getSolarResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = SolarRecipes.getInstance().getSolarResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
