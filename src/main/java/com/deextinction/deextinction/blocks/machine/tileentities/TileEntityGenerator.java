...
import com.deextinction.deextinction.blocks.machine.recipes.GeneratorRecipes;
...
public class TileEntityGenerator extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.generator");
	}
	...
	{	
		...
		{
			--this.burnTime;
			GeneratorBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = GeneratorRecipes.getInstance().getGeneratorResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = GeneratorRecipes.getInstance().getGeneratorResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
