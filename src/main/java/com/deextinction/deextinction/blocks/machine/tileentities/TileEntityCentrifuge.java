...
import com.deextinction.deextinction.blocks.machine.recipes.CentrifugeRecipes;
...
public class TileEntityCentrifuge extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.centrifuge");
	}
	...
	public static boolean isBurning(TileEntityCentrifuge te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			CentrifugeBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = CentrifugeRecipes.getInstance().getCentrifugeResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = CentrifugeRecipes.getInstance().getCentrifugeResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
