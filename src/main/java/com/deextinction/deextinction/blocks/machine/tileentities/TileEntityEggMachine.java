...
import com.deextinction.deextinction.blocks.machine.recipes.EggMachineRecipes;
...
public class TileEntityEggMachine extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.eggmachine");
	}
	...
	public static boolean isBurning(TileEntityEggMachine te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			EggMachineBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = EggMachineRecipes.getInstance().getEggMachineResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = EggMachineRecipes.getInstance().getEggMachineResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
	...
	public static int getItemBurnTime(ItemStack fuel) 
	{
		...
		else 
		{
			...
			if (item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR) 
			{
				...
				if (block == Blocks.REDSTONE_BLOCK) return 1800;
			}
