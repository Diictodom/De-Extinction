...
import com.deextinction.deextinction.blocks.machine.recipes.EstorageRecipes;
...
public class TileEntityEstorage extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.estorage");
	}
	...
	public static boolean isBurning(TileEntityEstorage te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			EstorageBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = EstorageRecipes.getInstance().getEstorageResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = EstorageRecipes.getInstance().getEstorageResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
