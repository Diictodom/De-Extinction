...
import com.deextinction.deextinction.blocks.machine.recipes.MicroscopeRecipes;
...
public class TileEntityMicroscope extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.microscope");
	}
	...
	public static boolean isBurning(TileEntityMicroscope te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			MicroscopeBlock.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = MicroscopeRecipes.getInstance().getMicroscopeResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = MicroscopeRecipes.getInstance().getMicroscopeResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
