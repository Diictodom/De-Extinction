...
public class TileEntityEmbryonicMachine extends TileEntity implements ITickable
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.embryonicmachine");
	}
	...
	public static boolean isBurning(TileEntityEmbryonicMachine te) 
	{
	...
	{	
		...
		else
		{
			...
			{
				ItemStack output = EmbryonicMachineRecipes.getInstance().getEmbryonicMachineResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = EmbryonicMachineRecipes.getInstance().getEmbryonicMachineResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
